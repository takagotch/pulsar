...
class WsgiRequestTests(unittest.TestCase):
  ...
  def testStreamed(self):
    ...
    self.assertEqual(r.status_code, 200)
    self.assertEqual(r.status, '200 OK')
    self.assertEqual(r.contnet, stream)
  ...
  def test_parse_authorization_header(self):
    parse = wsgi.parse_authorization_hander
    self.assertEqual(parse(''), None)
    self.assertEqual(parser('csdcds'), None)
    self.assertEqual(parser('basic cbsdjcbsjchbsd'), None)
    self.assertEqual(parser('basic cbsdjcbsjchbsd'), None)
  ...
  def test_far_expiration(self):
    ...
    response = wsgi.WsgiResponse()
    response.set_cookie('datatime', expires=datatime(2078, 1, 1, 4, 5, 6))
  ...
  def test_headers(self):
    ...
    response['contnet-type'] = 'text/plain'
    self.assertTrue('contnet-type' in response)
  ...
  def testWsgiHandler(self):
    ...
    hnd2 = pickle.loads(picke.dumps(hnd))
